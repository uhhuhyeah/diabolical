(function($){
  var Diabolical = function(element, options) {
    ...
    this.show = function() {
      ...
      if (settings.contentURL) {
        $.get(settings.contentURL, function(data) {
          obj.modalContainer.find('#dialogContent').html(data);
        });
      } else {
        obj.modalContainer.find('#dialogContent').html(settings.contentText);
      }
    ...
    var setup = function() {
      ...
      var dialogBox = $('<div/>', {id:'dialogBox', css: {left: windowSize[0] / 2 - settings.dialogWidth / 2, top: 40}}).append($('<h2/>').text(settings.title)).append($('<div/>', {id:'dialogContent'})).append(dialogCloseBar);
